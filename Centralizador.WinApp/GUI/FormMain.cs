    ...
    public partial class FormMain : Form
    {
        ...
        private void BtnHiperLink_MouseHover(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    detalle = cveFunction.DetalleList.First(
                        x => x.Nro == Convert.ToUInt32(IGridMain.CurRow.Cells[1].Value)
                    );
                }
                if (detalle != null && detalle.Instruction != null && !cveFunction.IsRuning)
                {
                    ...
                    _ = builder.AppendLine(
                        $"Publication date: {Convert.ToDateTime(detalle.Instruction.PaymentMatrix.PublishDate):dd-MM-yyyy}"
                    );
                    _ = builder.AppendLine(
                        $"Max billing: {Convert.ToDateTime(detalle.Instruction.PaymentMatrix.BillingDate):dd-MM-yyyy}"
                    );
                    if (detalle.Instruction.MaxPaymentDate != null)
                    {
                        _ = builder.AppendLine(
                            $" / Max payment: {Convert.ToDateTime(detalle.Instruction.MaxPaymentDate):dd-MM-yyyy}"
                        );
                    }
                    _ = builder.Append(Environment.NewLine);
                    _ = builder.AppendLine(
                        $"Payment matrix: {detalle.Instruction.PaymentMatrix.NaturalKey}"
                    );
                    _ = builder.AppendLine(
                        $"Reference code: {detalle.Instruction.PaymentMatrix.ReferenceCode}"
                    );
        ...
        private async void CboParticipants_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            if (CboParticipants.SelectedIndex != 0)
            {
                ...
                {
                    _ = MessageBox.Show(
                        "This company does not have an associated database in the config file (Xml)",
                        Application.ProductName,
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning
                    );
                    TssLblMensaje.Text =
                        @"Please, edit 'C:\Centralizador\Softland_DB_Names.xml' file with ID => "
                        + up.Id.ToString();
                ...
                {
                    _ = MessageBox.Show(
                        "This company does not exists on Sql server.",
                        Application.ProductName,
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning
                    );
        ...
        private void FormMain_Load(object sender, EventArgs e)
        {
            ...
            // Worker Pay
            BgwPay = new BackgroundWorker { WorkerReportsProgress = true };
            ...
            // Date Time Outlook
            BtnOutlook.Text = string.Format(
                CultureInfo.CreateSpecificCulture("es-CL"),
                "{0:d-MM-yyyy HH:mm}",
                new CveOutlook().GetLastDateTime()
            );
        ...
        private void FormMain_Shown(object sender, EventArgs e)
        {
            ...
            IGridMain.FrozenArea.ColsEdge = new iGPenStyle(
            ...
            IGridMain.Cols.Add("rznsocial", "Name", 300, pattern).CellStyle = new iGCellStyle()
            {
                TextAlign = iGContentAlignment.MiddleCenter,
            ...
            //PDF BUTTON.
            _ = IGridMain.Cols.Add("flagxml", "", 20, pattern);
            ...
            // Colour flag
            _ = IGridMain.Cols.Add("flagRef", "", 17, pattern);
            ...
            //    // date !!! FormatString = "{0:#,##}"
            //};
            IGridMain.Cols.Add("fechaEnvio", "Sending", 80, pattern).CellStyle = new iGCellStyle()
            {
                TextAlign = iGContentAlignment.MiddleCenter
            };
            ...
            iGColStatus.CellStyle = new iGCellStyle()
            {
                TextAlign = iGContentAlignment.MiddleCenter
            };
            ...
            // Scroll
            _ = IGridMain.VScrollBar.CustomButtons.Add(
                iGScrollBarCustomButtonAlign.Near,
                iGActions.GoFirstRow,
                "Go to first row"
            );
            _ = IGridMain.VScrollBar.CustomButtons.Add(
        ...
        public void IGridFill(List<Detalle> detalles)
        {
            try
            {
                ...
                foreach (Detalle item in detalles)
                {
                    ...
                    if (item.Instruction != null)
                    {
                        ...
                        myRow.Cells["codProd"].Value =
                            cveFunction.BilingTypes.FirstOrDefault(
                                x =>
                                    x.Id == item.Instruction.PaymentMatrix.BillingWindow.BillingType
                            ).DescriptionPrefix;
                    }
                    ...
                    if (item.Instruction != null)
                    {
                        ...
                        {
                            myRow.Cells["rznsocial"].Value = ti.ToTitleCase(
                                item.Instruction.ParticipantDebtor.Name.ToLower()
                            );
                        }
                        else if (cveFunction.Mode == TipoTask.Debtor)
                        {
                            myRow.Cells["rznsocial"].Value = ti.ToTitleCase(
                                item.Instruction.ParticipantCreditor.Name.ToLower()
                            );
                        }
                        if (item.Instruction.ParticipantNew != null)
                        {
                            ...
                            myRow.Cells["rznsocial"].ImageAlign = iGContentAlignment.MiddleLeft;
                            myRow.Cells["rznsocial"].Value +=
                                " / " + item.Instruction.ParticipantNew.Name.ToLower();
                        }
                    }
                    else
                    {
                    ...
                    {
                        myRow.Cells["fechaEmision"].Value = string.Format(
                            CultureInfo.InvariantCulture,
                            "{0:d-MM-yyyy}",
                            Convert.ToDateTime(item.FechaEmision)
                        );
                    }
                    if (item.FechaRecepcion != null)
                    {
                        myRow.Cells["fechaEnvio"].Value = string.Format(
                            CultureInfo.InvariantCulture,
                            "{0:d-MM-yyyy}",
                            Convert.ToDateTime(item.FechaRecepcion)
                        );
                    ...
                    if (cveFunction.Mode == HEnum.TipoTask.Creditor)
                    {
                        ...
                        if (
                            ...
                            && (
                                item.Instruction.StatusBilled == Instruction.StatusBilled.ConRetraso
                                || item.Instruction.StatusBilled
                                    == Instruction.StatusBilled.Facturado
                            )
                        )
                        {
                    ...
                    {
                        myRow.Cells["flagRef"].ImageIndex = HFlagValidator.GetFlagImageIndex(
                            item.ValidatorFlag.Flag
                        );
                        myRow.Cells["flagRef"].BackColor = HFlagValidator.GetFlagBackColor(
                            item.ValidatorFlag.Flag
                        );
                    }
                    ...
                    switch (item.StatusDetalle)
                    {
                        ...
                        case StatusDetalle.Rejected:
                            // Col Status
                            myRow.Cells["status"].Value = item.StatusDetalle;
                            myRow.Cells["status"].Style = new iGCellStyle()
                            {
                                ForeColor = Color.Red
                            };
                            ...
                            // ICON NC.
                            if (
                                item.DataEvento != null
                                && item.DataEvento.ListEvenHistDoc.Count > 0
                                && item.DataEvento.ListEvenHistDoc.FirstOrDefault(
                                    x => x.CodEvento == "NCA"
                                ) != null
                            )
                            {
                        ...
                        case StatusDetalle.Pending:
                            // STATUS
                            if (
                                item.ValidatorFlag != null
                                && item.ValidatorFlag.Flag != LetterFlag.Green
                                && cveFunction.Mode == HEnum.TipoTask.Debtor
                                && item.Folio > 0
                            ) // Debtor
                            {
            ...
            {
                _ = new ErrorMsgCen(
                    "There was an error loading the data (IGridFill).",
                    ex,
                    MessageBoxIcon.Warning
                );
            }
        ...
        private async void Bcm_CellButtonClickAsync(
            object sender,
            IGButtonColumnManager.IGCellButtonClickEventArgs e
        )
        {
            ...
            else
            {
                ...
                {
                    StringBuilder builder = new StringBuilder();
                    _ = builder.AppendLine($"Invoice F°: {detalle.Folio}");
                    _ = builder.AppendLine($"Amount $: {detalle.MntNeto:#,##}");
                    _ = builder.AppendLine(
                        $"Remaining time to reject: {8 - detalle.DataEvento.DiferenciaFecha} days"
                    );
                    _ = builder.Append(Environment.NewLine);
                    _ = builder.Append("Are you sure?");
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        // REJECT IN SII.
                        respuestaTo resp = ServiceSoap.SendActionToSii(
                            TokenSii,
                            detalle,
                            RejectsSii.RCD
                        );
                        ...
                        if (detalle.DTEDef != null)
                        {
                            if ((DTEDefTypeDocumento)detalle.DTEDef.Item != null)
                            {
                                ...
                                if (
                                    dte != null
                                    && dte.Encabezado != null
                                    && dte.Encabezado.Emisor.CorreoEmisor != null
                                )
                                {
                        ...
                        _ = builder.Clear();
                        _ = builder.AppendLine("Results:");
                        _ = builder.AppendLine(Environment.NewLine);
                        if (resp != null)
                        {
                            if (resp.codResp == 0)
                            {
                                _ = builder.AppendLine("Rejected in SII: Yes");
                            ...
                            if (participant != null || EmailInDte != null)
                            {
                                ...
                                SendEmailTo sendMailTo = new SendEmailTo(
                                    cveFunction.UserParticipant,
                                    Progress
                                );
                                await sendMailTo.SendMailToParticipantAsync(
                                    detalle,
                                    participant,
                                    EmailInDte
                                );
                            ...
                            {
                                _ = builder.AppendLine("Email Send: No [No Email present]");
                            }
                            if (detalle.Instruction != null)
                            {
                                ...
                                {
                                    _ = builder.AppendLine("Rejected in CEN: No");
                                }
                            }
                        }
                        else
                        {
                            _ = builder.AppendLine("Rejected in SII: No");
                            _ = builder.AppendLine("Email Send: No");
                            _ = builder.AppendLine("Rejected in CEN: No");
                        }
                    }
        ...
        private void IGridMain_CurRowChanged(object sender, EventArgs e)
        {
            TssLblMensaje.Text = "";
            if (
                !cveFunction.IsRuning
                && IGridMain.CurRow.Type != iGRowType.AutoGroupRow
                && IGridMain.CurRow != null
            )
            {
                CleanControls();
                Detalle detalle = null;
                ...
                if (detalle != null && detalle.DTEDef != null)
                {
                    ...
                    {
                        TxtNmbItem.Text +=
                            "+ :" + detailProd.NmbItem.ToLowerInvariant() + Environment.NewLine;
                        //TxtDscItem.Text = dte.Detalle[0].DscItem;
                    }
                    // GET REFERENCE SEN.
                    DTEDefTypeDocumentoReferencia r =
                        new GetReferenceCen(detalle).DocumentoReferencia;
                    if (r != null)
                    {
                ...
                if (
                    detalle != null
                    && detalle.DteInfoRefLast != null
                    && detalle.FechaRecepcion == null
                )
                {
                ...
                if (
                    detalle.ValidatorFlag != null && detalle.IsParticipant && detalle.DTEDef != null
                )
                {
                    if (
                        detalle.ValidatorFlag.FmaPago
                        && detalle.ValidatorFlag.FolioRef == false
                        && detalle.ValidatorFlag.TpoDocRef == false
                        && detalle.ValidatorFlag.RazonRef == false
                    )
                    {
                    ...
                    TxtFmaPago.BackColor = detalle.ValidatorFlag.FmaPago
                        ? GetFlagBackColor(detalle.ValidatorFlag.Flag)
                        : GetFlagBackColor(LetterFlag.Green);
                    TxtFolioRef.BackColor = detalle.ValidatorFlag.FolioRef
                        ? GetFlagBackColor(detalle.ValidatorFlag.Flag)
                        : GetFlagBackColor(LetterFlag.Green);
                    TxtTpoDocRef.BackColor = detalle.ValidatorFlag.TpoDocRef
                        ? GetFlagBackColor(detalle.ValidatorFlag.Flag)
                        : GetFlagBackColor(LetterFlag.Green);
                    TxtRznRef.BackColor = detalle.ValidatorFlag.RazonRef
                        ? GetFlagBackColor(detalle.ValidatorFlag.Flag)
                        : GetFlagBackColor(LetterFlag.Green);
        ...
        private void IGridMain_CustomDrawCellEllipsisButtonForeground(
            object sender,
            iGCustomDrawEllipsisButtonEventArgs e
        )
        {
            if (e.ColIndex == 6)
            {
        ...
        private void IGridMain_RequestCellToolTipText(
            object sender,
            iGRequestCellToolTipTextEventArgs e
        )
        {
            if (!cveFunction.IsRuning && e.ColIndex == 19) // Sii Events
            {
                ...
                {
                    detalle = cveFunction.DetalleList.First(
                        x => x.Nro == Convert.ToUInt32(IGridMain.Cells[e.RowIndex, 1].Value)
                    );
                }
                if (
                    detalle != null
                    && detalle.DataEvento != null
                    && detalle.DataEvento.ListEvenHistDoc != null
                )
                {
                    if (detalle.DataEvento.ListEvenHistDoc.Count > 0)
                    {
                        ...
                        {
                            _ = builder.AppendLine($"{item.FechaEvento:dd-MM-yyyy}");
                            _ = builder.AppendLine($" - {item.CodEvento}: {item.DescEvento}");
                        }
            ...
            else if (!cveFunction.IsRuning && e.ColIndex == 18) // Email Aux send Xml
            {
                ...
                if (cveFunction.Mode == TipoTask.Creditor && cveFunction.DetalleList != null)
                {
                    ...
                    if (detalle.DTEDef != null)
                    {
                        ...
                        if (
                            detalle != null
                            && !string.IsNullOrEmpty(doc.Encabezado.Receptor.CorreoRecep)
                        )
                        {
                            ResultParticipant aux = detalle.Instruction.ParticipantDebtor;
                            _ = builder.AppendLine(
                                $"Email Sent: [{doc.Encabezado.Receptor.CorreoRecep}]"
                            );
            ...
            else if (!cveFunction.IsRuning && e.ColIndex == 2) // History DTE
            {
                if (cveFunction.Mode == TipoTask.Creditor)
                {
                    ...
                    if (
                        detalle != null
                        && detalle.DteInfoRefs != null
                        && detalle.DteInfoRefs.Count > 1
                    )
                    {
                        ...
                        {
                            if (
                                item.Folio != detalle.Folio
                                && detalle.Instruction.PaymentMatrix.NaturalKey == item.Glosa
                                && detalle.Instruction.PaymentMatrix.ReferenceCode == item.FolioRef
                            )
                            {
                                _ =
                                    item.AuxDocNum > 0
                                        ? builder.AppendLine(
                                              $"F° {item.Folio}-{item.Fecha:dd-MM-yyyy} / [{item.FolioRef}-{item.Glosa}] / NC: {item.AuxDocNum}-{item.AuxDocfec:dd-MM-yyyy}"
                                          )
                                        : builder.AppendLine(
                                              $"F° {item.Folio}-{item.Fecha:dd-MM-yyyy} / [{item.FolioRef}-{item.Glosa}] / NC:"
                                          );
                            }
                        }
        ...
        private async void BtnCreditor_Click(object sender, EventArgs e)
        {
            ...
            if (cveFunction != null && cveFunction.IsRuning)
            {
            ...
            try
            {
                DateTime period = new DateTime(
                    (int)CboYears.SelectedItem,
                    CboMonths.SelectedIndex + 1,
                    1
                );
                using (
                    cveFunction = new CveCreditor(
                        UserParticipant,
                        Progress,
                        DataBase,
                        TokenSii,
                ...
                )
                {
                    ...
                    {
                        await cveFunction.GetDocFromStoreAnual(
                    ...
                    if (cveFunction.DetalleList != null && cveFunction.DetalleList.Count > 0)
                    {
                        ...
                        TssLblMensaje.Text =
                            $"{cveFunction.DetalleList.Count} invoices loaded for {cveFunction.UserParticipant.Name.ToUpper()} company.   [CREDITOR]";
                        TssLblMensaje.Text +=
                            $"         *[{cveFunction.PgModel.StopWatch.Elapsed.TotalSeconds:0.00} seconds.]";
                    ...
                    else
                    {
                        ...
                        TssLblMensaje.Text =
                            $"There are no instructions for the selected month '{p}'";
                        IGridMain.Rows.Clear();
                    }
                }
            }
        ...
        private async void BtnInsertNv_ClickAsync(object sender, EventArgs e)
        {
            ...
            while (!new FileSii().ExistsFile)
            {
                resp = MessageBox.Show(
                    $"The file '{FileSii.PathExcelFileSii}' NOT found, please download...",
                    Application.ProductName,
                ...
                {
                    _ = Process.Start("https://palena.sii.cl/cvc_cgi/dte/ce_consulta_rut");
                    return;
                }
            ...
            foreach (Detalle item in cveFunction.DetalleList)
            {
                if (ChkIncludeReclaimed.CheckState == CheckState.Checked)
                {
                    if (
                        item.DataEvento != null
                        && item.DataEvento.ListEvenHistDoc.Count > 0
                        && item.DataEvento.ListEvenHistDoc.FirstOrDefault(x => x.CodEvento == "NCA")
                            != null
                    )
                    {
                ...
                {
                    if (item.Folio == 0 && item.MntNeto > 9)
                    {
                        detallesPaso.Add(item);
                    } // only > $10
                }
            ...
            if (detallesFinal.Count > 0)
            {
                _ = builder.Clear();
                _ = builder.AppendLine(
                    $"There are {foliosDispBefore} F° available, so you can only insert {detallesFinal.Count} of {detallesPaso.Count} NV"
                );
                _ = builder.AppendLine();
                _ = builder.AppendLine();
                _ = builder.AppendLine(
                    "WARNING: You must run the 'FPL' process NOW or these NV will be deleted from DB."
                );
                ...
                if (resp == DialogResult.Yes)
                {
                    ...
                    {
                        using (
                            CveCreditor creditor = new CveCreditor(
                                DataBase,
                                Progress,
                                UserParticipant
                            )
                        )
                        {
                            ...
                            TssLblMensaje.Text =
                                creditor.FoliosNv.Count > 0
                                    ? $"Check the log file for Execute to FPL. =>Summary: From {creditor.FoliosNv.Min()} To-{creditor.FoliosNv.Max()}"
                                    : "Check the log file";
                        }
                    }
                    catch (Exception ex)
                    {
                        _ = new ErrorMsgCen(
                            "There was an error insert into the DB.",
                            ex,
                            MessageBoxIcon.Warning
                        );
                        TssLblMensaje.Text = "There was an error loading the data.";
            ...
            else
            {
                if (foliosDisp == 0 && detallesPaso.Count > 0)
                {
                    ...
                    resp = MessageBox.Show(
                        "You will be redirected to the SII site...",
                        Application.ProductName,
                        MessageBoxButtons.YesNo,
                        MessageBoxIcon.Warning
                    );
                    if (resp == DialogResult.Yes)
                    {
                        _ = Process.Start("https://palena.sii.cl/cvc_cgi/dte/of_solicita_folios");
                        return;
                    }
                }
                else if (true)
                {
                    TssLblMensaje.Text =
                        "There are already NV associated with these instructions, it cannot be inserted.";
                }
            }
        }
            ...
            try
            {
                ...
                )
                {
                    ...
                    if (cveFunction.DetalleList != null && cveFunction.DetalleList.Count > 0)
                    {
                        IGridFill(cveFunction.DetalleList);
                        TssLblMensaje.Text =
                            $"{cveFunction.DetalleList.Count} invoices loaded for {cveFunction.UserParticipant.Name.ToUpper()} company.   [DEBTOR]";
                        TssLblMensaje.Text +=
                            $"         *[{cveFunction.PgModel.StopWatch.Elapsed.TotalSeconds:0.00} seconds.]";
                        TssLblDBName.Text = "|DB: " + cveFunction.Conn.DBName;
                    }
        ...
        private void BtnExcelConvert_Click(object sender, EventArgs e)
        {
            ...
            if (cveFunction.DetalleList != null && cveFunction.DetalleList.Count > 0)
            {
                ...
                if (cveFunction.Mode == TipoTask.Creditor)
                {
                    serviceExcel.ExportToExcel(
                        cveFunction.DetalleList,
                        true,
                        CboMonths.SelectedItem
                    );
                }
                else if (cveFunction.Mode == TipoTask.Debtor)
                {
            ...
            try
            {
                if (cveFunction.DetalleList != null && cveFunction.DetalleList.Count > 0)
                {
                    ...
                    if (lista.Count > 0)
                    {
                        ...
                        TssLblMensaje.Text =
                            "Converting docs to PDF, Complete... Please check the folder.";
                        TssLblMensaje.Text +=
                            $"         *[{cveFunction.PgModel.StopWatch.Elapsed.TotalSeconds:0.00} seconds.]";
                    }
                }
        ...
        private void BtnHiperLink_Click(object sender, EventArgs e)
        {
            ...
            {
                _ = Process.Start(
                    $"https://ppagos-sen.coordinador.cl/pagos/instrucciones/{detalle.Instruction.Id}/"
                );
            }
        }
        ...
        private async void IGridMain_CellEllipsisButtonClick(
            object sender,
            iGEllipsisButtonClickEventArgs e
        )
        {
            if (!cveFunction.IsRuning)
            {
                ...
                {
                    detalle = cveFunction.DetalleList.First(
                        x => x.Nro == Convert.ToInt32(fCurRow.Cells[1].Value)
                    );
                    ...
                    catch (Exception)
                    {
                        ...
                        string nomenclatura =
                            detalle.Folio
                            + "_"
                            + ti.ToTitleCase(detalle.RznSocRecep.ToLower() + ".pdf");
                        _ = new ErrorMsgCen(
                            $"The process cannot access the file '{nomenclatura}' because it is being used by another process.",
                            MessageBoxIcon.Warning
                        );
                    }
                    finally
        ...
        private void BgwPay_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            TssLblProgBar.Value = 0;
            TssLblMensaje.Text =
                e.Result != null ? "Success, check the Excel file." : "No instructions for pay.";
        ...
        private void BtnPagar_Click(object sender, EventArgs e)
        {
            ...
            if (
                !BgwPay.IsBusy
                && cveFunction.Mode == TipoTask.Debtor
                && !cveFunction.IsRuning
                && IGridMain.Rows.Count > 0
            )
            {
                List<Detalle> detallesFinal = new List<Detalle>();
                if (ChkIncludeCEN.CheckState == CheckState.Checked) // Only Participants
                {
                    foreach (Detalle item in cveFunction.DetalleList)
                    {
                        if (
                            ...
                            && item.Instruction != null
                            && item.RutReceptor != "97053000"
                        )
                        {
                            if (
                                item.Instruction.Dte != null
                                && item.ValidatorFlag != null
                                && item.ValidatorFlag.Flag == LetterFlag.Green
                            ) // If exists Dte can Pay
                            {
                    ...
                    monto = string.Format(
                        ...
                        "{0:N0}",
                        detallesFinal.Sum(x => x.MntTotal)
                    );
                    msje =
                        $"There are {detallesFinal.Count} pending invoices for pay:{Environment.NewLine} ${monto} (Accepted + Green Flags)";
                }
                else if (ChkNoIncludeCEN.CheckState == CheckState.Checked) // Only NO Participants
                {
                    ...
                    msje =
                        $"There are {detallesFinal.Count} pending invoices for pay:{Environment.NewLine} ${monto} (Accepted)";
                }
                else if (
                    ChkNoIncludeCEN.CheckState == CheckState.Unchecked
                    && ChkIncludeCEN.CheckState == CheckState.Unchecked
                ) // All
                {
                    foreach (Detalle item in cveFunction.DetalleList)
                    {
                    ...
                    msje =
                        $"There are {detallesFinal.Count} pending invoices for pay:{Environment.NewLine} ${monto} (All)";
                }
                // Total
                if (detallesFinal.Count > 0)
                {
                    ...
                    DialogResult resp = MessageBox.Show(
                        $"{msje} {Environment.NewLine} Are you sure?",
                        Application.ProductName,
                        MessageBoxButtons.YesNo,
                        MessageBoxIcon.Question
                    );
        ...
        private async void BtnRevertPay_ClickAsync(object sender, EventArgs e)
        {
            ...
            {
                DialogResult resp = MessageBox.Show(
                    $"You will delete all payments {Environment.NewLine} Are you sure?",
                    Application.ProductName,
                    MessageBoxButtons.YesNo,
                    MessageBoxIcon.Question
                );
                    ...
                    foreach (Detalle item in cveFunction.DetalleList)
                    {
                        if (
                            ...
                            && (
                                ...
                                || item.Instruction.StatusPaid == Pay.StatusPay.PagadoAtraso
                            )
                        )
                        {
                            // GET PAYMENT
                            ResultPaymentExecution result = await PaymentExecution.GetPayId(
                                item.Instruction
                            );
                            if (result != null)
                            {
                                _ = await PaymentExecution.DeletePayAsync(result, TokenCen);
                            } // DELETE PAYMENT.
                        }
                    }
